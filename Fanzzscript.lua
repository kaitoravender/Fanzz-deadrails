-- FANZZ DEAD RAILS SUITE - ULTIMATE (FINAL)
-- by FanzzSenpai
-- Fitur: Auto Bonds (500-3000), Fake Hitbox (visual), Real Hitbox (damage, anti-freeze STABLE)
-- Fix: Team check, delay aman, posisi label, mobile-friendly minimize
...
bodyFrame.Size = UDim2.new(1, -12, 0, 265)
...
statusFake.Position = UDim2.new(0, 10, 0, 256)
...
statusReal.Position = UDim2.new(0, 10, 0, 272)
...
bondLabel.Position = UDim2.new(0, 10, 0, 288)
...
progressLabel.Position = UDim2.new(0, 10, 0, 230)  -- FIX: tidak numpuk dengan statusFake
...
task.spawn(function()    
    ...
    local endRange = 3000    
    local delay = 0.05  -- FIX: lebih aman dari deteksi spam (dari 0.025 jadi 0.05)    
...
-- FIX: isEnemy sekarang membedakan team
...
if plr then    
    -- Jika player memiliki team, cek perbedaan team    
    return plr.Team ~= player.Team    
end    
    
-- Non-player (NPC/monster) dianggap musuh    
return true
...
-- ========== REAL HITBOX (ANTI-FREEZE STABLE) ==========
...
local newX = math.min(head.Size.X * scale, 6)
local newZ = math.min(head.Size.Z * scale, 6)
...
fakeLabel.Position = UDim2.new(0, 10, 0, 58)
...
createModeButton(bodyFrame, 12, "3x", Color3.fromRGB(60,60,70), function()
stopRealHitbox()
startFakeHitbox(3)
end).Position = UDim2.new(0, 12, 0, 82)

createModeButton(bodyFrame, 85, "5x", Color3.fromRGB(60,60,70), function()
stopRealHitbox()
startFakeHitbox(5)
end).Position = UDim2.new(0, 85, 0, 82)

createModeButton(bodyFrame, 158, "10x", Color3.fromRGB(60,60,70), function()
stopRealHitbox()
startFakeHitbox(10)
end).Position = UDim2.new(0, 158, 0, 82)

createModeButton(bodyFrame, 85, "20x", Color3.fromRGB(60,60,70), function()
stopRealHitbox()
startFakeHitbox(20)
end).Position = UDim2.new(0, 85, 0, 122)

createModeButton(bodyFrame, 158, "OFF", Color3.fromRGB(120,40,40), function()
stopFakeHitbox()
end).Position = UDim2.new(0, 158, 0, 122)

-- ========== REAL HITBOX BUTTONS ==========
local realLabel = Instance.new("TextLabel")
realLabel.Size = UDim2.new(1, -20, 0, 18)
realLabel.Position = UDim2.new(0, 10, 0, 150)
